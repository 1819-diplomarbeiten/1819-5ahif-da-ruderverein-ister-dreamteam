...
// initialize object
    ...
    function getBestFourDistances($email, $year)
    {
        global $db;
        global $query;
        ...
        from (select participant_email, distance 
              from result 
              where participant_email = :email and challenge_id%10000 =:year
...
// initialize object
    ...
    function emailExists($email)
    {
        ...
        if ($result == null){
            $sql = $db->prepare("SELECT * from club where club.email = :email");
    ...
    function getClubContraction($club){
        global $db;
        global $query;

        $sql = $db->prepare("SELECT contraction from club where name =:club");
        $sql->bindValue(':club', $club, PDO::PARAM_STR);
        $result = json_decode($query->buildJson("contraction", "con", $sql), true);
        return $result[0]['con'];
    }

    function getParticipantData($email){
        global $db;
        global $query;

        $sql = $db->prepare("SELECT * from participant where email =:email");
        $sql->bindValue(':email', $email, PDO::PARAM_STR);
        $result = json_decode($query->buildJson("first_name", "firstName", $sql), true);
        if($result == null)
            return null;
        $result = $result[0];
    ...
    function checkIfChallengeNeedsUpdate($year){
        global $db;
        global $query;

        $sql = $db->prepare("SELECT * from challenge where challenge_id%10000 =:year");
        $sql->bindValue(':year', $year, PDO::PARAM_INT);
        $result = json_decode($query->buildJson("challenge_id", "id", $sql), true);
        if($result == null){
            return false;
    ...
    function getClubsWithoutAdmin(){
        global $db;

        $sql = $db->prepare("SELECT name from club where email like '%@nomail.com%'");
        $sql->execute();
    ...
    function resultAlreadyExists($challengeId, $email){
        global $db;

        $sql = $db->prepare("SELECT result_id from result where participant_email =:email AND challenge_id=:challenge_id");
        $sql->bindValue(':email',$email,PDO::PARAM_STR);
        $sql->bindValue(':challenge_id',$challengeId,PDO::PARAM_INT);
        $sql->execute();
        while ($row = $sql->fetch(PDO::FETCH_ASSOC)) {
