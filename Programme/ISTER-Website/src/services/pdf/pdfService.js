...
var participationColumns = [
    {title: "Teilnahme", dataKey:"participation"},
    {title: "1st", dataKey:"roundOne"},
    {title: "2nd", dataKey:"roundTwo"},
    {title: "3rd", dataKey:"roundThree"},
    {title: "4th", dataKey:"roundFour"},
...
var categoryColumns = [
    {title: "Klasse", dataKey:"category"},
    {title: "Year", dataKey:"years"},
...
export default class PdfService{
    ...
    static createEmailName(result){
        ...
        var columns = emailDistanceColumns
        var rows = []
        for(var j = 0; j < result.length;j++){
            rows[j] = {
    ...
    static getDistanceFootnoteArray(rows, result, startIndex){
        ...
        rows[startIndex + 5] = {
            "clubLong": "Ergebnis pro Wochenende",
    ...
    static createParticipationTable(doc, result){
        var doc = this.createStatisticsSubheader(doc, "Teilnahme", 30)
        var columns = participationColumns
        var rows = []
        rows[0] = this.getSingleParticipationRow(result.female, "weiblich")
        rows[1] = this.getSingleParticipationRow(result.male, "männlich")
        rows[2] = this.getSingleParticipationRow(result.total, "total")
        doc.autoTable(columns, rows, this.getClubPdfAutotableOptions(40, 13, 11));
        return doc;
    }
    ...
    static createCategoryTable(doc, result){
        ...
        doc = this.createStatisticsSubheader(doc, "Kategorien", doc.autoTableEndPosY() + 25)
        var rows = []
        for(var j = 0; j < result.length;j++){
            rows[j] = {
    ...
    static getSessionColumns(session, pClass){
        if(pClass == "")
            return [
                ...
                {title: "Round " + session, dataKey:"round"},
                {title: "", dataKey:"empty"},
